...
template<> struct AbstractTexture::DataHelper<1> {
    ...
    inline static void setWrapping(Target target, const Math::Vector<Wrapping, 1>& wrapping) {
        glTexParameteri(Corrade::Utility::castToUnderlyingType(target), GL_TEXTURE_WRAP_S, Corrade::Utility::castToUnderlyingType(wrapping[0]));
    }

    template<class T> inline static void set(Target target, GLint mipLevel, InternalFormat internalFormat, T* image) {
        glTexImage1D(Corrade::Utility::castToUnderlyingType(target), mipLevel, internalFormat, image->dimensions()[0], 0, Corrade::Utility::castToUnderlyingType(image->components()), Corrade::Utility::castToUnderlyingType(image->type()), image->data());
    }

    template<class T> inline static void setSub(Target target, GLint mipLevel, const Math::Vector<GLint, 1>& offset, T* image) {
        glTexSubImage1D(Corrade::Utility::castToUnderlyingType(target), mipLevel, offset[0], image->dimensions()[0], Corrade::Utility::castToUnderlyingType(image->components()), Corrade::Utility::castToUnderlyingType(image->type()), image->data());
    }
};
template<> struct AbstractTexture::DataHelper<2> {
    ...
    inline static void setWrapping(Target target, const Math::Vector<Wrapping, 2>& wrapping) {
        glTexParameteri(Corrade::Utility::castToUnderlyingType(target), GL_TEXTURE_WRAP_S, Corrade::Utility::castToUnderlyingType(wrapping[0]));
        glTexParameteri(Corrade::Utility::castToUnderlyingType(target), GL_TEXTURE_WRAP_T, Corrade::Utility::castToUnderlyingType(wrapping[1]));
    }

    template<class T> inline static void set(Target target, GLint mipLevel, InternalFormat internalFormat, T* image) {
        glTexImage2D(Corrade::Utility::castToUnderlyingType(target), mipLevel, internalFormat, image->dimensions()[0], image->dimensions()[1], 0, Corrade::Utility::castToUnderlyingType(image->components()), Corrade::Utility::castToUnderlyingType(image->type()), image->data());
    }

    template<class T> inline static void setSub(Target target, GLint mipLevel, const Math::Vector<GLint, 2>& offset, T* image) {
        glTexSubImage2D(Corrade::Utility::castToUnderlyingType(target), mipLevel, offset[0], offset[1], image->dimensions()[0], image->dimensions()[1], Corrade::Utility::castToUnderlyingType(image->components()), Corrade::Utility::castToUnderlyingType(image->type()), image->data());
    }
};
template<> struct AbstractTexture::DataHelper<3> {
    ...
    inline static void setWrapping(Target target, const Math::Vector<Wrapping, 3>& wrapping) {
        glTexParameteri(Corrade::Utility::castToUnderlyingType(target), GL_TEXTURE_WRAP_S, Corrade::Utility::castToUnderlyingType(wrapping[0]));
        glTexParameteri(Corrade::Utility::castToUnderlyingType(target), GL_TEXTURE_WRAP_T, Corrade::Utility::castToUnderlyingType(wrapping[1]));
        glTexParameteri(Corrade::Utility::castToUnderlyingType(target), GL_TEXTURE_WRAP_R, Corrade::Utility::castToUnderlyingType(wrapping[2]));
    }

    template<class T> inline static void set(Target target, GLint mipLevel, InternalFormat internalFormat, T* image) {
        glTexImage3D(Corrade::Utility::castToUnderlyingType(target), mipLevel, internalFormat, image->dimensions()[0], image->dimensions()[1], image->dimensions()[2], 0, Corrade::Utility::castToUnderlyingType(image->components()), Corrade::Utility::castToUnderlyingType(image->type()), image->data());
    }

    template<class T> inline static void setSub(Target target, GLint mipLevel, const Math::Vector<GLint, 3>& offset, T* image) {
        glTexSubImage3D(Corrade::Utility::castToUnderlyingType(target), mipLevel, offset[0], offset[1], offset[2], image->dimensions()[0], image->dimensions()[1], image->dimensions()[2], Corrade::Utility::castToUnderlyingType(image->components()), Corrade::Utility::castToUnderlyingType(image->type()), image->data());
    }
};
#endif
